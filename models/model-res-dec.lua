...
local function decode(iFeatures, oFeatures, stride, adjS)
   ...
   mainBlock:add(nn.SpatialFullConvolution(iFeatures, oFeatures, 3, 3, stride, stride, 1, 1, adjS, adjS))
   mainBlock:add(SBatchNorm(oFeatures, 1e-3))
   mainBlock:add(nn.ReLU(true))
   mainBlock:add(Convolution(oFeatures, oFeatures, 1, 1, 1, 1, 0, 0))
   mainBlock:add(SBatchNorm(oFeatures, 1e-3))

   local residual = nn.Sequential()
   residual:add(Convolution(iFeatures, oFeatures, 1, 1, 1, 1, 0, 0))
   residual:add(SBatchNorm(oFeatures, 1e-3))
   residual:add(nn.SpatialUpSamplingBilinear(stride))

   -- Initialization
   for i = 1, 2 do
      ConvInit(mainBlock:get(1))
      ConvInit(mainBlock:get(4))
      ConvInit(residual:get(1))

      BNInit(mainBlock:get(2))
      BNInit(mainBlock:get(5))
      BNInit(residual:get(2))
   end
...
local function layer(layerN, features) -- , block, features, count, stride)
   iChannels = features
...
local function bypass2dec(features, layers, stride, adjS)
   ...
   -- local bypass = nn.Sequential()   -- Bypass to decoder
   local oFeatures = iChannels

   accum:add(prim):add(nn.Identity())

   -- Add the bottleneck modules
   prim:add(layer(layers, features)) --block, features, 2, stride))
   if layers == 4 then
      --DECODER
